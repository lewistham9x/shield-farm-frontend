...
const DepositSection = ({ pool }) => {
  ...
  const { fetchDeposit, fetchDepositBnb, fetchDepositPending } =
    useFetchDeposit();
  const { fetchZapDeposit } = useFetchZapDeposit();
  const { balances, tokenBalance, fetchBalances } = useFetchBalances();
  const { fetchZapDepositEstimate, fetchZapEstimatePending } =
    useFetchZapEstimate();
  ...
  const [depositSettings, setDepositSettings] = useState({
    ...
    isNeedApproval: new BigNumber(
      balances[eligibleTokens[0].symbol].allowance[pool.earnContractAddress]
    ).isZero(),
  ...
  useEffect(() => {
    const allowance = new BigNumber(
      balances[depositSettings.token.symbol].allowance[
        depositSettings.depositAddress
      ]
    );
  ...
  }, [depositSettings.depositAddress, depositSettings.token.symbol, balances]);
  ...
  const handleTokenChange = (event) => {
    ...
    const allowance = new BigNumber(balances[token.symbol].allowance[spender]);
  ...
  const handleInputAmountChange = (event) => {
    ...
    const sliderInt = total.isZero()
      ? 0
      : amount.times(100).dividedToIntegerBy(total).toNumber();
    const allowance = new BigNumber(
      balances[depositSettings.token.symbol].allowance[
        depositSettings.depositAddress
      ]
  ...
  const depositAssets = (deposit) => {
    ...
    if (deposit.isZap) {
      ...
      fetchZapDeposit(zapDepositArgs)
        ...
        .catch((error) =>
          enqueueSnackbar(t("Vault-DepositError", { error }), {
            variant: "error",
          })
        );
    } else {
    ...
    if (status === "eol") {
      ...
      cont = (
        <div className={classes.showDetailButtonCon}>
          <div className={classes.showRetiredMsg}>
  ...
  const swapTokenOut = depositSettings.isZap
    ? eligibleTokens.find(
        (t) =>
          ...
          pool.zapEstimate?.swapTokenOut?.toLowerCase()
      )
    : undefined;
  ...
  return (
    ...
    >
            ...
            pool.zapEstimate && (
              <div className={classes.zapNote}>
                ...
                <ol>
                  ...
                  <li>
                    {t("Vault-DepositScenarioDepositToVault", {
                      poolToken: pool.token,
                    })}
                  </li>
                  <li>
      ...
      <p className={classes.note}>
        {t("Vault-DepositTokensNote", {
          mooToken: pool.earnedToken,
          assetToken: pool.token,
        })}
      </p>
    </Grid>
  );
